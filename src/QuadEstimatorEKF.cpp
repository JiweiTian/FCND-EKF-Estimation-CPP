...
//  if (ekfState(6) < -F_PI) ekfState(6) += 2.f*F_PI;
    ...
    // yaw = atan2(d.X, d.Z)
    ekfState(6) = atan2f(sinf(q.Yaw()), cosf(q.Yaw()));
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    V3F accelIntl = attitude.Rotate_BtoI(accel) - V3F(0, 0, CONST_GRAVITY);
    
    for (int i = 0; i < 3; ++i)  //- position integral
    {
        predictedState(i) += curState(i+3) * dt;
        predictedState(i+3) += accelIntl[i] * dt;
    }
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    hPrime(6) = 1;
    zFromX(0) = ekfState(6);
    float diffYaw = z(0) - ekfState(6);
    
    if (diffYaw > F_PI) zFromX(0) += 2.f*F_PI;
    if (diffYaw < -F_PI) zFromX(0) -= 2.f*F_PI;
